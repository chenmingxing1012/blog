...
- -gccause：看上次 GC、本次 GC（如果正在 GC 中）的原因，其他输出和-gcutil选项一致。
...
jstat -gcutil -t 864
```
-gcutil选项是统计 GC 相关区域的使用率（utilization），结果如下：
Timestamp
S0
S1
E
...
0.122
206.158
-t选项的位置是固定的，不能在前也不能在后。可以看出是用于显示时间戳，即 JVM 启动到现在的秒数。
简单分析一下：
...
可以看到，-gcutil这个选项出来的信息不太好用，统计的结果是百分比，不太直观。
再看看-gc选项，GC 相关的堆内存信息。
```
jstat -gc -t 864 1s
...
......
```
简单分析，其中[C占用了 11MB 内存，没占用什么空间。
[C表示chat[]，[B表示byte[]，[I表示int[]，其他类似。这种基础数据类型很难分析出什么问题。
...
在 Linux 和 macOS 上，jstack pid的效果跟kill -3 pid相同。
### jinfo 工具
...
![](assets/780663e0-2608-11ea-aa89-e1cacb5bf377)
然后就可以看到如下信息：
```
jinfo 36663
