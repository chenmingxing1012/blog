...
```
其实，-Xdebug这个选项什么用都没有，官方说是为了历史兼容性，避免报错才没有删除。
另外这个参数配置里的suspend=y会让 Java 进程启动时先挂起，等到有调试器连接上以后继续执行程序。
而如果改成suspend=n的话，则此 Java 进程会直接执行，但是我们可以随时通过调试器连上进程。
...
![](assets/5adef100-2c99-11ea-8c87-e5afec28626d)
添加 Remote（不是 Tomcat 下面的那个 Remote Server）：
![](assets/6d83bcf0-2c99-11ea-8c87-e5afec28626d)
然后配置端口号，比如 8888。
![](assets/7e457100-2c99-11ea-b3f5-45352c445237)
然后点击应用（Apply）按钮。
