---
title: 13 工具进阶：如何利用 MAT 找到问题发生的根本原因
breadcrumb: false
---
...
![]\(assets/Cgq2xl5NRsiAGcDYAAGoA74s9J4470.jpg)
...
path to GC Roots这是快速分析的一个常用功能，显示和 GC Roots 之间的路径。

![]\(assets/CgpOIF5NRsiAF2UoAACXuKqZ_nE086.jpg)

另外一个比较重要的概念，就是浅堆（Shallow Heap）和深堆（Retained Heap），在 MAT 上经常看到这两个数值。

![]\(assets/Cgq2xl5NRsmADmSUAADHVcry3i0679.jpg)
...
![]\(assets/CgpOIF5NRsmAEeWTAABDIx8RWa4815.png)
...
我们以一段代码示例Objects4MAT，来具体看一下 MAT 工具的使用。代码创建了一个新的线程 "huge-thread"，并建立了一个引用的层级关系，总的内存大约占用 100 MB。同时，demo1 和 demo2 展示了一个循环引用的关系。最后，使用 sleep 函数，让线程永久阻塞住，此时整个堆处于一个相对“静止”的状态。

![]\(assets/Cgq2xl5NRsmAC91XAABbohWGa5g179.png)

如果你是在本地启动的示例代码，则可以使用 Accquire 的方式来获取堆快照。

![]\(assets/CgpOIF5NRsmAe6KpAADnQ4RiJB0172.jpg)

### 2.2. 内存泄漏检测

如果问题特别突出，则可以通过 Find Leaks 菜单快速找出问题。

![]\(assets/Cgq2xl5NRsmAXR8iAADltd6bubA950.jpg)

如下图所示，展示了名称叫做 huge-thread 的线程，持有了超过 96% 的对象，数据被一个 HashMap 所持有。

![]\(assets/CgpOIF5NRsmAY8daAACqOrzPq-0668.jpg)
...
![]\(assets/Cgq2xl5NRsmAK6e-AAHIVUhKKVo936.jpg)

从支配树视图同样能够找到我们创建的两个循环依赖，但它们并没有显示这个过程。

![]\(assets/CgpOIF5NRsmANxVwAAEHUj82zng151.jpg)

支配树视图的概念有一点点复杂，我们只需要了解这个概念即可。

![]\(assets/Cgq2xl5NRsmATfB_AACo8ZKxSls467.png)
...
![]\(assets/CgpOIF5NRsqAL2HLAAC_bwnowFA744.jpg)
...
![]\(assets/Cgq2xl5NRsqAG2bqAAE8ffV3Y6A856.jpg)
...
如下图，我们找到了 huge-thread，依次展开找到 holder 对象，可以看到循环依赖已经陷入了无限循环的状态。这在查看一些 Java 对象的时候，经常发生，不要感到奇怪。![]\(assets/CgpOIF5NRsqAGh2eAAFzWwgR_5s636.jpg)
...
![]\(assets/CgpOIF5NRsuAFXehAAC0TcFd7Zs280.jpg)
...
![]\(assets/Cgq2xl5NRsuAMARGAACW6OEtUPs910.jpg)
...
![]\(assets/CgpOIF5NRsuAP-J5AAEx2z68KT4959.jpg)
