---
title: 04 动手实践：从栈帧看字节码是如何在 JVM 中进行流转的
breadcrumb: false
---
...
![](assets/CgpOIF4ezuOAK_6bAACFY5oeX-Y174.jpg)
...
可以看到对象的初始化，首先是调用了 Object 类的初始化方法。注意这里是而不是
...
![](assets/Cgq2xl4ezeKAWB30AADZXqT3TjQ870.jpg)
...
方法。以下截图是 test 方法的 code 区域，比命令行版的更加直观。

![](assets/CgpOIF4ezeKAVmSnAACExsXdgtg544.jpg)
...
![](assets/Cgq2xl4ezeKASWJHAAB5Ptt1JsM137.jpg)
...
![](assets/CgpOIF4ezeKAHVCXAABv7rzSgXE896.jpg)
...
![](assets/CgpOIF4w-GGAA6DnAAEtqWkdOnE696.jpg)
...
![](assets/Cgq2xl4w-HKABrhgAAEvNAmbGWY870.jpg)
...
![](assets/CgpOIF4w-IuAOmp0AAEzFWM0gmc155.jpg)

（5）ladd

把栈顶两个 long 型数值出栈后相加，并将结果入栈。

![](assets/Cgq2xl4w-KKAGhwcAAEtNkzwpcw021.jpg)

（6）getstatic #3

根据偏移获取静态属性的值，并把这个值 push 到操作数栈上。

![](assets/Cgq2xl4w-MWAVt_ZAAE2NxokOfU153.jpg)

（7）ladd

再次执行 ladd。

![](assets/CgpOIF4w-NCAaU4rAAEtel-Iskk153.jpg)
...
![](assets/CgpOIF4w-OWAPOn9AAE1Y2sXttM659.jpg)

（9）lload_3

正好与上面相反。上面是变量存入，我们现在要做的，就是把这个变量 ret，压入虚拟机栈中。

![](assets/CgpOIF4w-O6ARdRFAAE62GkvYGo689.jpg)
